...
<article class="swap-rates" ng-show="showStage1">
  ...
  <!-- Colored Columns -->
  <section ng-if="!isKyberSwap" class="row order-panel">
  ...
  <!-- Kyber (and Dev Injections) -->
  <section ng-if="isKyberSwap">
    <!-- Colored Columns -->
    <section class="row order-panel">
        <h3>KYBER</h3>
      <div class="col-sm-6 order-info">
        <p ng-repeat="(key, value) in kyberPriceTicker" ng-if="$even && kyber.kyberRates[key] != 0" class="mono">
          {{$index}}
          <input class="form-control input-sm" ng-model="kyberPriceTicker[key]"/>
          <span>{{kyber.kyberRates[key].slice(0,3)}} = {{kyber.kyberRates[key].rate*kyberPriceTicker[key] | number: 6}} {{kyber.kyberRates[key].slice(3))}}</span>
        </p>
      </div>
      <div class="col-sm-6 order-info">
        <p ng-repeat="(key, value) in kyberPriceTicker" ng-if="$odd  && kyber.currentRates[key].rate != 0" class="mono">
          {{$index}}
          <input class="form-control input-sm" ng-model="kyberPriceTicker[key]"/>
          <span>{{kyber.currentRates[key].pair[0]}} = {{kyber.currentRates[key].rate*kyberPriceTicker[key] | number: 6}} {{kyber.currentRates[key].pair[1]}}</span>
        </p>
      </div>
      <a class="link bity-logo" href="#" target="_blank" rel="noopener noreferrer"><img src="images/logo-kyber.png" width="120" height="49" /></a>
    </section>
    <!-- / Colored Columns -->
