...
var walletGenCtrl = function($scope) {
	...
	$scope.genNewWallet = function() {
		if (!$scope.isStrongPass()) {
			...
			$scope.blob = globalFuncs.getBlob("application/octet-stream", $scope.wallet.toJSON());
			$scope.blobEnc = globalFuncs.getBlob("application/octet-stream", $scope.wallet.toV3($scope.password, {
				kdf: globalFuncs.kdf,
