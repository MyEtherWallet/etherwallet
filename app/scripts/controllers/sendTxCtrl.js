    ...
    $scope.sendTx = function() {
        ...
        uiFuncs.sendTx($scope.signedTx, function(resp) {
            if (!resp.isError) {
                ...
                var checkTXLink = '<a class="btn btn-xs btn-info" href="#check-tx-status" target="_blank" rel="noopener"> Check TX Status </a>';
                var emailLink = '<a class="btn btn-xs btn-info" href="mailto:support@myetherwallet.com?Subject=Issue%20regarding%20my%20TX%20&Body=I%20was%20trying%20to..............%0A%0A%0A%0ABut%20I%27m%20confused%20because...............%0A%0A%0A%0A%0A' + "%0ATo%20Address%3A%20https%3A%2F%2Fetherscan.io%2Faddress%2F" + $scope.tx.to + "%0AFrom%20Address%3A%20https%3A%2F%2Fetherscan.io%2Faddress%2F" + $scope.wallet.getAddressString() + "%0ATX%20Hash%3A%20https%3A%2F%2Fetherscan.io%2Ftx%2F" + resp.data + "%0AAmount%3A%20" + $scope.tx.value + "%20" + $scope.unitReadable + "%0ANode%3A%20" + $scope.ajaxReq.type + "%0AToken%20To%20Addr%3A%20" + $scope.tokenTx.to + "%0AToken%20Amount%3A%20" + $scope.tokenTx.value + "%20" + $scope.unitReadable + "%0AData%3A%20" + $scope.tx.data + "%0AGas%20Limit%3A%20" + $scope.tx.gasLimit + "%0AGas%20Price%3A%20" + $scope.tx.gasPrice + '" target="_blank" rel="noopener">Confused? Email Us.</a>';
                var completeMsg = "<p>" + globalFuncs.successMsgs[2] + '<strong>' + resp.data + "</strong></p><p>" + bExStr + " " + checkTXLink + " " + emailLink + "</p>";
