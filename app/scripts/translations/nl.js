...
x_PasswordDesc              : 'Dit wachtwoord * versleuteld * je prive sleutel (`private key`). Dit wachtwoord dient niet als `seed` om je sleutels te genereren. **Je hebt zowel je wachtwoord als je prive sleutel (`private key`) beide nodig om je wallet te openen.**',
...
tx_Summary                  : 'Ten tijden van hoog transactie volume (zoals tijdens ICO`s) kunnen transacties voor uren, zo niet niet dagen, `pending` blijven. Deze tool heeft als doel om je de mogelijkheid te geven om deze TX`s op te sporen en ze te `annuleren` / vervangen. ** Dit is niet typisch iets wat je kunt doen. Je zou er niet op moeten vertrouwen dat dit werkt & het kan alleen werken als de TX Pools vol zitten. [Lees hier meer over deze tool.](https://myetherwallet.groovehq.com/knowledge_base/topics/how-can-i-check-on-the-status-of-my-transaction-can-i-cancel-override-overwrite-replace-or-do-anything-once-a-transaction-has-been-sent)**',
...
GAS_PRICE_Desc              : 'Gas Prijs is het bedrag dat je betaald per unit gas. `TX vergoeding = gas prijs * gas limiet` & wordt betaald aan de miners om jouw TX in een blok op de blockchain op te nemen. Des te hoger de gas prijs = de sneller je transactie, maar ook duurder. Standaard is `21 GWEI`.',
...
GEN_Label_2                 : 'Sla je Keystore bestand of Prive Sleutel (`Private Key`) op. Vergeet je wachtwoord hierboven niet. ',
